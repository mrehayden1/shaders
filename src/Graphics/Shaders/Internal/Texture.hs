...
loadTexture :: (MonadAsyncException m, MonadLogger m) => ShadersT m Texture
loadTexture = do
  deviceHandle <- getDeviceHandle

  debug "Loading texture image."
  TGA{..} <- decodeFile "texture-uncompressed.tga"
  ...
  liftIO . withForeignPtr imageBytes $ \ptr ->
    copyBytes ptr (castPtr bufferPtr) imageNumBytes
  VkMemory.unmapMemory deviceHandle stagingBufferMemory

  debug "Creating image."
  (image, imageMemory) <- createImage
    tgaWidth
    tgaHeight
